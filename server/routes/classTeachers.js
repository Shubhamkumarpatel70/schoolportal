...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const classTeachers = await ClassTeacher.find().populate('teacherId', 'name email staffId');
...
router.get('/class/:className', auth, async (req, res) => {
  try {
    const classTeacher = await ClassTeacher.findOne({ className: req.params.className }).populate('teacherId', 'name email phone staffId');
    if (!classTeacher) {
      return res.status(404).json({ message: 'Class teacher not found' });
    }
    res.json(classTeacher);
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const populated = await ClassTeacher.findById(classTeacher._id).populate('teacherId', 'name email staffId');
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    const classTeacher = await ClassTeacher.findByIdAndUpdate(req.params.id, req.body, { new: true }).populate('teacherId', 'name email staffId');
    if (!classTeacher) {
      return res.status(404).json({ message: 'Class teacher not found' });
    }
    res.json(classTeacher);
